...
function loadImage(src: string): Promise<HTMLImageElement> {
    return new Promise((resolve, reject) => {
        ...
        img.onerror = (err) => reject(new Error(`فشل تحميل التصويرة: ${src.substring(0, 50)}...`));
        img.src = src;
    });
}
...
export async function createAlbumPage(imageData: Record<string, string>): Promise<string> {
    ...
    if (!ctx) {
        throw new Error('مقدرناش ناخدو سياق الكانفاس ثنائي الأبعاد');
    }
    ...
    ctx.font = `bold 100px 'Aref Ruqaa', 'Caveat', cursive`;
    ctx.fillText('ذكريات عبر الزمن', canvasWidth / 2, 150);

    ctx.font = `50px 'Tajawal', 'Roboto', sans-serif`;
    ctx.fillStyle = '#555';
    ctx.fillText('على Google AI Studio', canvasWidth / 2, 220);
    ...
    reversedImages.forEach(({ decade, img }, reversedIndex) => {
        ...
        ctx.fillStyle = '#222';
        ctx.font = `60px 'Aref Ruqaa', 'Permanent Marker', cursive`;
        ctx.textAlign = 'center';
